...
export const getPlatformStats = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const [totalUsers, totalPings, totalSurges, totalWaves, totalComments, totalOrganizations] =
      await prisma.$transaction([
        prisma.user.count({
          where: { organizationId: req.organizationId!, ...(createdAtFilter ?? {}) },
        }),
        prisma.ping.count({
          where: { organizationId: req.organizationId!, ...(createdAtFilter ?? {}) },
        }),
...
export const getPingsByLevel = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const statsByLevel = pings.reduce(
      (acc, ping) => {
...
export const getPingStatsByCategory = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
...
export const updatePingProgressStatus = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
...
export const updateWaveStatusAsAdmin = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
...
export const getPriorityPings = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const scored = candidates
      ...
      .sort(
        (a, b) => b.priorityScore - a.priorityScore || b.createdAt.getTime() - a.createdAt.getTime()
      )
      .slice(0, limit);
...
export const getPingSentimentAnalytics = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
...
export const exportPingsAsCsv = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="pings_export_${new Date().toISOString().split('T')[0]}.csv"`
    );
...
export const getAdminOverviewDashboard = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const [recentSurges, stalledWaves, wavesAwaitingApproval, stalledAcknowledgedPings] =
      await prisma.$transaction([
        prisma.surge.groupBy({
          by: ['pingId'],
          where: {
    ...
    const categoriesStats = categoryRows.map((category) => {
      ...
      const resolved = category.pings.filter(
        (ping) => ping.progressStatus === ProgressStatus.RESOLVED
      ).length;
    ...
    const resolutionRateCurrent = pingsCurrentRows.length
      ? (pingsCurrentRows.filter((ping) => ping.progressStatus === ProgressStatus.RESOLVED).length /
          pingsCurrentRows.length) *
        100
      : 0;
    const resolutionRatePrevious = pingsPreviousRows.length
      ? (pingsPreviousRows.filter((ping) => ping.progressStatus === ProgressStatus.RESOLVED)
          .length /
          pingsPreviousRows.length) *
        100
      : 0;
    ...
    const chartBuckets = new Map<
      string,
      { month: string; waves: number; pings: number; resolved: number }
    >();
        ...
        return {
          ...
          author: ping?.author
            ? {
                id: ping.author.id,
                name:
                  `${ping.author.firstName ?? ''} ${ping.author.lastName ?? ''}`.trim() ||
                  'Unknown',
              }
            : null,
    ...
    const categoryHealth = categoryRows
      ...
      .sort(
        (a, b) =>
          b.unresolvedCount - a.unresolvedCount || a.categoryName.localeCompare(b.categoryName)
      );
      ...
      summaryCards: {
        ...
        resolutionRate: {
          ...
          deltaPercentagePoints: Number(
            (resolutionRateCurrent - resolutionRatePrevious).toFixed(2)
          ),
        },
        avgResolveTimeDays: {
...
export const getCommunityMood = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const trend = Array.from({ length: days }, (_, index) => {
      ...
      return {
        date: key,
        positivePercent: dayBucket.total
          ? Number(((dayBucket.positive / dayBucket.total) * 100).toFixed(2))
          : 0,
        neutralPercent: dayBucket.total
          ? Number(((dayBucket.neutral / dayBucket.total) * 100).toFixed(2))
          : 0,
        negativePercent: dayBucket.total
          ? Number(((dayBucket.negative / dayBucket.total) * 100).toFixed(2))
          : 0,
...
export const getActivityTimeSeries = async (
  ...
  try {
    ...
    const buckets: Record<
      string,
      { date: string; pings: number; waves: number; resolved: number }
    > = {};
...
export const unsuspendMemberAsAdmin = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
...
export const removeMemberAsAdmin = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: 'Member removed from organization successfully.', userId: targetUserId });
  } catch (error) {
    return next(error);
  }
};
...
export const updateOrganizationRules = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const updated = await prisma.organization.update({
      ...
      data: {
        ...
        ...(autoFlagReportThreshold !== undefined && {
          reportsAutoFlagThreshold: autoFlagReportThreshold,
        }),
        ...(hideFlaggedContentPending !== undefined && {
          hideFlaggedPendingReview: hideFlaggedContentPending,
        }),
...
export const getFollowUpQueue = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const staleDays =
      Number.isFinite(staleDaysParam) && staleDaysParam > 0 ? Math.min(staleDaysParam, 365) : 7;
    const staleDate = new Date(now.getTime() - staleDays * MS_IN_DAY);

    const [approvedWavesNotImplementing, wavesAwaitingApproval, acknowledgedPingsStalling] =
      await Promise.all([
