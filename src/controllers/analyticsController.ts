...
export const getAdminOverview = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const wavesUnderReview = await prisma.wave.count({
    ...
    const [
      ...
      prisma.ping.count({
        where: { organizationId, status: 'POSTED', createdAt: { gte: thirtyDaysAgo } },
      }),
      prisma.wave.count({
        where: { organizationId, status: 'UNDER_REVIEW', createdAt: { gte: thirtyDaysAgo } },
      }),
      prisma.user.count({
        where: { organizationId, status: 'ACTIVE', createdAt: { gte: thirtyDaysAgo } },
      }),
      prisma.wave.count({
    ...
    const formattedChartData = chartData.map(({ month, thisYear, lastYear }) => ({
      month,
      thisYear,
      lastYear,
    }));
    ...
    res.json({
      summaryCards: {
        ...
        wavesUnderReview: {
          total: wavesUnderReview,
          percentChange: calcPercentChange(currentReviewWaves, prevReviewWaves),
        },
        activeUsers: {
          total: activeUsers,
          percentChange: calcPercentChange(currentUsers, prevUsers),
        },
      },
...
// @access  Private
export const getPingLevelAnalytics = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  try {
    ...
    res.json({
      ...
      surgeBreakdown: Object.entries(surgeBreakdown).map(([level, count]) => ({
        level,
        count,
        percentage: surges.length > 0 ? (count / surges.length) * 100 : 0,
      })),
      totalComments: comments.length,
      commentBreakdown: Object.entries(commentBreakdown).map(([level, count]) => ({
        level,
        count,
        percentage: comments.length > 0 ? (count / comments.length) * 100 : 0,
      })),
      totalWaves: waves.length,
      waveBreakdown: Object.entries(waveBreakdown).map(([level, count]) => ({
        level,
        count,
        percentage: waves.length > 0 ? (count / waves.length) * 100 : 0,
      })),
    });
  } catch (error) {
