...
export default function errorHandler(
  ...
  if (err instanceof Error) {
    if (err.name === 'JsonWebTokenError') {
      return res
        .status(401)
        .json({ error: 'Invalid authentication token.', code: 'JWT_INVALID', requestId });
    }
  ...
  // Default fallback
  const message =
    err instanceof Error ? err.message : 'Something went wrong on our end. Please try again later.';
  ...
  return res
    .status(500)
    .json({ error: 'Something went wrong on our end. Please try again later.', requestId });
}
