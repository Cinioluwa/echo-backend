...
it('should return 200 OK', async () => {
  const res = await client.get('/api/my-endpoint').set('Authorization', `Bearer ${token}`);
...
2.  **Prisma Client:** Never import the global `prisma` from `src/config/db.ts`. Always use `getPrisma()` from `./testContainer.ts`. The global one connects to the _production_ DB (or fails if unreachable).
